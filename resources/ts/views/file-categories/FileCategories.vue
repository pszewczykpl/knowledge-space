    ...
    <SearchPanel>
        ...
        <template v-slot:toolbar>
            <div v-if="selectedIds.length === 0" class="d-flex justify-content-end">
                <router-link :to="{ name: 'FileCategoryCreate' }" class="btn btn-primary">
        ...
        <template v-slot:default>
            ...
            >
                ...
                <template v-slot:description="{ row: row }"> {{ row.prefix }} </template>
                <template v-slot:actions="{ row: row }">
                    <router-link
                        :to="{ name: 'FileCategoryUpdate', params: {id: row.id } }"
...
const tableHeader = ref([
    ...
    { columnName: "Prefix", columnLabel: "prefix", sortEnabled: true },
    { columnName: "Akcje", columnLabel: "actions", sortEnabled: false },
]);
const { loading, dataTable, initDataTable, selectedIds, search, onItemSelect, searchI, deleteElements, deleteElement } = useDataTable('fileCategories', 'fileCategory');
