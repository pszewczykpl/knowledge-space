    ...
    <SearchPanel>
        ...
        <template v-slot:toolbar>
            <div v-if="selectedIds.length === 0" class="d-flex justify-content-end">
                <router-link :to="{ name: 'LinkCreate' }" class="btn btn-primary">
        ...
        <template v-slot:default>
            ...
            >
                ...
                <template v-slot:description="{ row: row }"> {{ row.description }} </template>
                <template v-slot:url="{ row: row }">
                    <a :href="row.url" target="_blank">{{ row.url }}</a>
                </template>
                <template v-slot:actions="{ row: row }">
                    <router-link
                        :to="{ name: 'LinkUpdate', params: {id: row.id } }"
...
const tableHeader = ref([
    { columnName: "Nazwa linku", columnLabel: "name", sortEnabled: true },
    { columnName: "Opis", columnLabel: "description", sortEnabled: true },
    { columnName: "URL", columnLabel: "url", sortEnabled: true },
    { columnName: "Akcje", columnLabel: "actions", sortEnabled: false },
]);
const { loading, dataTable, initDataTable, selectedIds, search, onItemSelect, searchI, deleteElements, deleteElement } = useDataTable('links', 'link');
