        ...
        @can('create', App\Models\Note::class)
            {!! Form::open(['route' => 'notes.store', 'method' => 'post']) !!}
                <div class="mb-0">
                    ...
                    <textarea class="form-control form-control-solid placeholder-gray-600 fw-bolder fs-4 ps-9 pt-7" rows="4" name="content" id="content" placeholder="Treść notatki"></textarea>
                    <button type="submit" class="btn btn-primary mt-n20 mb-20 position-relative float-end me-7">Dodaj notatkę</button>
                </div>
            {!! Form::close() !!}
                ...
                <div class="card card-bordered w-100">
                    <div class="card-body">
                        <div class="d-flex flex-stack mb-5">
                            ...
                            <div class="m-0">
                                @can('update', $note)
                                    <a href="{{ route('notes.edit', $note->id) }}" class="btn btn-light btn-sm">Edytuj</a>
                                @endcan
                                @can('update', $note)
                                    <a href="{{ route('notes.detach', ['note' => $note, 'noteable_type' => $noteable_type, 'noteable_id' => $noteable_id]) }}" class="btn btn-light btn-sm">Odepnij</a>
                                @endcan
                                @can('delete', $note)
                                    <a onclick='document.getElementById("note_delete_form_{{ $note->id }}").submit();' class="btn btn-danger btn-sm">Usuń</a>
                                @endcan
                            </div>
                        </div>
