...
<div class="container-fluid d-flex align-items-center justify-content-between flex-wrap flex-sm-nowrap">
	...
	<div class="d-flex align-items-center">
        <a href="{{ route('file-categories.index') }}" class="btn btn-clean btn-sm mr-1">@include('svg.back', ['class' => 'navi-icon']) Powrót</a>
        @can('update', $file_category)
        <a href="{{ route('file-categories.edit', $file_category->id) }}" class="btn btn-light-primary btn-sm mr-1">@include('svg.edit', ['class' => 'navi-icon']) Edytuj</a>
        @endcan
        @can('delete', $file_category)
            <a onclick='document.getElementById("file_categories_destroy_{{ $file_category->id }}").submit();' class="btn btn-light-danger btn-sm">@include('svg.trash', ['class' => 'navi-icon']) Usuń</a>
