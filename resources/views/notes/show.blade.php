...
<div class="container-fluid d-flex align-items-center justify-content-between flex-wrap flex-sm-nowrap">
	...
	<div class="d-flex align-items-center">
        <a href="{{ route('notes.index') }}" class="btn btn-clean btn-sm mr-1">@include('svg.back', ['class' => 'navi-icon']) Powrót</a>
        @can('update', $note)
        <a href="{{ route('notes.edit', $note->id) }}" class="btn btn-light-primary btn-sm mr-1">@include('svg.edit', ['class' => 'navi-icon']) Edytuj</a>
        @endcan
        @can('delete', $note)
            <a onclick='document.getElementById("notes_destroy_{{ $note->id }}").submit();' class="btn btn-light-danger btn-sm">@include('svg.trash', ['class' => 'navi-icon']) Usuń</a>
