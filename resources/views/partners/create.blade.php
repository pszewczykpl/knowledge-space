...
@section('toolbar')
        <a href="{{ route('partners.index') }}" class="btn btn-clean btn-sm mr-1">@include('svg.back', ['class' => 'navi-icon']) Anuluj</a>
        @can('create', App\Models\Partner::class)
        <a onclick='document.getElementById("partner_store_form").submit();' class="btn btn-light-primary btn-sm">@include('svg.save', ['class' => 'navi-icon']) Zapisz</a>
        @endcan
@stop
