...
<div class="container-fluid d-flex align-items-center justify-content-between flex-wrap flex-sm-nowrap">
	...
	<div class="d-flex align-items-center">
		<a href="{{ route('risks.index') }}" class="btn btn-clean btn-sm mr-1">@include('svg.back', ['class' => 'navi-icon']) Powrót</a>
        <a onclick="ShareRisks('{{ $risk->id }}')" class="btn btn-light-primary btn-sm mr-1">@include('svg.share', ['class' => 'navi-icon']) Udostępnij</a>
		@can('update', $risk)
			<a href="{{ route('risks.edit', $risk->id) }}" class="btn btn-light-primary btn-sm mr-1">@include('svg.edit', ['class' => 'navi-icon']) Edytuj</a>
		@endcan
		@can('create', App\Models\Risk::class)
			<a href="{{ route('risks.duplicate', $risk) }}" class="btn btn-light-primary btn-sm mr-1">@include('svg.duplicate', ['class' => 'navi-icon']) Duplikuj</a>
		@endcan
		@can('delete', $risk)
			<a onclick='document.getElementById("risks_destroy_{{ $risk->id }}").submit();' class="btn btn-light-danger btn-sm">@include('svg.trash', ['class' => 'navi-icon']) Usuń</a>
