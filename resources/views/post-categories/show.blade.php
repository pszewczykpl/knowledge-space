...
<div class="container-fluid d-flex align-items-center justify-content-between flex-wrap flex-sm-nowrap">
	...
	<div class="d-flex align-items-center">
        <a href="{{ route('post-categories.index') }}" class="btn btn-clean btn-sm mr-1">@include('svg.back', ['class' => 'navi-icon']) Powrót</a>
        @can('update', $post_category)
        <a href="{{ route('post-categories.edit', $post_category->id) }}" class="btn btn-light-primary btn-sm mr-1">@include('svg.edit', ['class' => 'navi-icon']) Edytuj</a>
        @endcan
        @can('delete', $post_category)
            <a onclick='document.getElementById("post_categories_destroy_{{ $post_category->id }}").submit();' class="btn btn-light-danger btn-sm">@include('svg.trash', ['class' => 'navi-icon']) Usuń</a>
