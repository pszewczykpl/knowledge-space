...
@section('toolbar')
        <a href="{{ route('post-categories.show', $post_category) }}" class="btn btn-clean btn-sm mr-1">@include('svg.back', ['class' => 'navi-icon']) Anuluj</a>
        @can('update', $post_category)
            <a onclick='document.getElementById("post_cateogry_update_form").submit();' class="btn btn-light-primary btn-sm">@include('svg.save', ['class' => 'navi-icon']) Zapisz</a>
        @endcan
@stop
